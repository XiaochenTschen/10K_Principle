...
// Constances for the packet protocol
...
// The whole packet state machine runs here.
uart.on('data', data => {
    ...
    if(rxBuffer.length >= PACKET_LENGTH) {
        ...
        if(packet.isAck()) {
            console.log(`Ack, nothing to do`);
            return;
        }

        // otherwise write the packet in to the buffer, and send an ack
        console.log(`Storing packet and acking`);
        packets.push(packet);
...
const main = async () => {
    console.log('Waitging for packet ...');
